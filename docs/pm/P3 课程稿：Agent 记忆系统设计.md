...
时效性管理里还有一个产品级难题：**当新旧信息矛盾时，系统该怎么处理？**

比如”吃素“和”喜欢意大利菜“两条记忆的遗忘分数各自独立变化，但检索时可能同时被想起。这个时候就需要额外的冲突策略：

| 策略 | 做法 | 适用场景 | 风险 |
| --- | --- | --- | --- |
| **并行降权** | 新旧信息都保留，各自随时间和使用情况调整分数 | 偏好可能是阶段性的，如短期饮食调整 | 检索时两条都出现，需要综合判断 |
| **显式覆盖** | 新信息明确取代旧信息，旧条目的分数立刻大幅下调 | 用户表达了确定性的变更 | 若用户只是随口一说，可能误伤仍有价值的旧偏好 |

没有绝对正确的答案。并行降权更贴近真实人类助理的行为；显式覆盖更适合确定性的变更。

更好的产品设计，是在两种策略之间留一个**用户确认的接口**：检测到潜在冲突时，主动问一句——你最近开始吃素了，之前的意大利菜偏好还需要保留吗？把裁决权交给用户，既避免误删，也强化信任。
...
3. **思考四个设计问题**：
   ...
   - 当新旧偏好冲突时（比如最近在吃素 vs 喜欢意大利菜），你的产品会采用并行降权还是显式覆盖？
